...
const atlasPath = path.join(root, 'subpackages', 'game', 'native', 'd6', 'd614ee96-d20f-4373-8064-5d92fd589021.ac70a.atlas');
const atlasTexturePath = path.join(root, 'subpackages', 'game', 'native', '6b', '6bc5fc93-b403-4b31-a0e1-7ccff54c45d7.7b5a8.png');
const outDir = path.join(root, 'animal_art_extracted', 'preview_variants', 'sheep_blue_native_atlas_spine_render_preview');
...
function cropAtlasRegion(texture, region) {
  const packedW = region.rotate ? region.height : region.width;
  const packedH = region.rotate ? region.width : region.height;
  const top = texture.height - region.y - packedH;
  const dst = blank(packedW, packedH);
  for (let y = 0; y < packedH; y++) {
    for (let x = 0; x < packedW; x++) setPixel(dst, x, y, getPixel(texture, region.x + x, top + y));
  }
  return dst;
}
...
function main() {
  ...
  const atlasTexture = loadPng(atlasTexturePath);
  const parts = {};
  const packedParts = {};
  for (const name of sheepDrawOrder) {
    const packedPart = cropAtlasRegion(atlasTexture, atlas[name]);
    packedParts[name] = packedPart;
    parts[name] = atlas[name].rotate ? rotateCcw(packedPart) : packedPart;
  }
  ...
  fs.mkdirSync(path.join(outDir, 'parts_logical'), { recursive: true });
  fs.mkdirSync(path.join(outDir, 'parts_packed_from_atlas'), { recursive: true });
  ...
  for (const name of sheepDrawOrder) {
    savePng(path.join(outDir, 'parts_packed_from_atlas', `${name}.png`), packedParts[name]);
    savePng(path.join(outDir, 'parts_logical', `${name}.png`), parts[name]);
  }

  const renderedFrames = [];
  for (let frame = 0; frame < frameCount; frame++) {
    const time = duration * frame / frameCount;
    const rendered = renderFrame(skeleton, atlas, parts, time, view);
    renderedFrames.push(rendered);
    savePng(path.join(outDir, 'frames', `frame_${String(frame).padStart(2, '0')}.png`), rendered);
    savePng(path.join(outDir, 'white_bg_frames', `frame_${String(frame).padStart(2, '0')}.png`), compositeOnBackground(rendered, [255, 255, 255, 255]));
    savePng(path.join(outDir, 'checker_bg_frames', `frame_${String(frame).padStart(2, '0')}.png`), compositeOnChecker(rendered));
  }

  savePng(path.join(outDir, 'full_sheep_preview', 'blue_native_sheep_yidle_frame_00.png'), renderedFrames[0]);
  savePng(path.join(outDir, 'full_sheep_preview', 'blue_native_sheep_yidle_24_frame_contact_sheet.png'), makeContactSheet(renderedFrames, 6, 10));
  savePng(path.join(outDir, 'full_sheep_preview', 'blue_native_sheep_yidle_24_frame_contact_sheet_white_bg.png'), makeContactSheet(renderedFrames.map(frame => compositeOnBackground(frame, [255, 255, 255, 255])), 6, 10));
  savePng(path.join(outDir, 'full_sheep_preview', 'blue_native_sheep_yidle_24_frame_contact_sheet_checker_bg.png'), makeContactSheet(renderedFrames.map(frame => compositeOnChecker(frame)), 6, 10));

  const lines = [
    'Current game blue sheep native-atlas Spine render preview.',
    '',
    'This preview is built from the current game sheep atlas texture plus the original Spine skeleton.',
    'It does not depend on extracted sheep part folders or a full bitmap overlay.',
    'Atlas rotate/orig/offset values are applied, then yidle skeleton bone transforms are sampled for 24 frames.',
    'The preview script itself does not replace game resources; it renders the already-updated game atlas.',
    '',
    `skeleton=${path.relative(root, skeletonPath).replace(/\\/g, '/')}`,
    `atlas=${path.relative(root, atlasPath).replace(/\\/g, '/')}`,
    `atlas_texture=${path.relative(root, atlasTexturePath).replace(/\\/g, '/')}`,
  ...
  for (const name of sheepDrawOrder) {
    const region = atlas[name];
    lines.push(`${name}: atlas_rotate=${region.rotate}, atlas_size=${region.width}x${region.height}, atlas_orig=${region.originalWidth}x${region.originalHeight}, atlas_offset=${region.offsetX},${region.offsetY}, packed_from_atlas=${packedParts[name].width}x${packedParts[name].height}, logical_render_part=${parts[name].width}x${parts[name].height}`);
  }
  fs.writeFileSync(path.join(outDir, 'README.txt'), lines.join('\n'));
  console.log(`Wrote ${outDir}`);
  console.log(`frame_size=${view.width}x${view.height}`);
