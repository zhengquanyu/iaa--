...
const atlasTexturePath = path.join(root, 'animal_art_extracted', 'level_spine', 'original_cocos_files', 'animal.png');
const outDir = path.join(root, 'animal_art_extracted', 'preview_variants', 'sheep_original_spine_render_export');
...
function main() {
  ...
  savePng(path.join(outDir, 'full_sheep_preview', 'original_sheep_yidle_frame_00.png'), renderedFrames[0]);
  savePng(path.join(outDir, 'full_sheep_preview', 'original_sheep_yidle_24_frame_contact_sheet.png'), makeContactSheet(renderedFrames, 6, 10));
  savePng(path.join(outDir, 'full_sheep_preview', 'original_sheep_yidle_24_frame_contact_sheet_white_bg.png'), makeContactSheet(renderedFrames.map(frame => compositeOnBackground(frame, [255, 255, 255, 255])), 6, 10));
  savePng(path.join(outDir, 'full_sheep_preview', 'original_sheep_yidle_24_frame_contact_sheet_checker_bg.png'), makeContactSheet(renderedFrames.map(frame => compositeOnChecker(frame)), 6, 10));

  const lines = [
    'Original sheep Spine render export.',
    '',
    'This export is built directly from original animal.png + animal.atlas plus animal.skeleton.json.',
    'It does not depend on animal_art_extracted/level_spine/animals_by_body/小羊, because that folder may contain packed/rotated or stale extracted pieces.',
    'Atlas rotate/orig/offset values are applied, then yidle skeleton bone transforms are sampled for 24 frames.',
    'No game resource was replaced by this export script.',
    '',
